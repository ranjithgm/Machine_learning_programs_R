...
summarise(group_by(customer_churn,InternetService),mean_MonthlyCharges=mean(MonthlyCharges))
###
...
customer_churn %>% group_by(PaymentMethod) %>% summarise(mean_tenure=mean(tenure)) -> new_df
